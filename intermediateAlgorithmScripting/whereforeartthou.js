...
//input array of objects - collection
//name and value - source (match to this)
//returns an array of all objects that mach the name and value pairs of source
...
function whatIsInAName(collection, source) {
   ...
   for(let i = 0; i < collection.length; i++){
       for(let key in source){
           for(let key2 in collection[i]){
               if(key === key2){
                   if(source[key] === collection[i][key2]){
