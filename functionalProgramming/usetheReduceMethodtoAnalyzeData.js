...
function movieTotal(watchList){

    watchList = watchList.filter(arr => arr.Director === 'Christopher Nolan')
    let numMovies = watchList.length;

    let totalRating = watchList.reduce((acc, currentvalue) =>{
        return acc + +(currentvalue.imdbRating);
    }, 0)

    return totalRating / numMovies;
}

let averageRating = movieTotal(watchList);
